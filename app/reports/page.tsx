...
const fetchLDNominationData = async (): Promise<RowData[]> => {
  try {
    const response = await axios.get("http://localhost:3000/LDNominationData.json");
    const data = response.data;
    ...
    return data.map((row: RowData) => ({
      ...row,
      nominationDate: new Date(row.nominationDate),
...
const Page = async () => {
  const rows = await fetchLDNominationData(); // Fetch the data on the server

  return (
    <div>
      <LdNominationTable initialRows={rows} />
    </div>
  );
};
